...
namespace lyra2
{
	...
	public class HTML : Form
	{
		...
		private void updateFields(string url)
		{
			...
			this.textBox2.Text = url + sep + this.project + "\\lyra2.html";
			...
			if (Directory.Exists(this.textBox3.Text) || File.Exists(this.textBox2.Text))
			{
				DialogResult dr =
					MessageBox.Show(this, "Achtung! Sie überschreiben möglicherweise eine bereits erstellte HTML-Datei!",
					                "lyra2 Hinweis", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
		...
		private string songstyle = "\tspan.special\n\t\t\t{\n\t\t\t\tcolor:#000000;\n\t\t\t\tfont-family:times;\n\t\t\t\tfont-style:italic;\n\t\t\t}\n\n\t\t\t" +
			"span.p8\n\t\t\t{\n\t\t\t\tpadding-left:16px;\n\t\t\t}\n\n\t\t\t" +
			"span.p16\n\t\t\t{\n\t\t\t\tpadding-left:32px;\n\t\t\t}\n\n\t\t\t" +
			"span.p24\n\t\t\t{\n\t\t\t\tpadding-left:48px;\n\t\t\t}\n\n\t\t\t" +
			"span.p32\n\t\t\t{\n\t\t\t\tpadding-left:64px;\n\t\t\t}\n\n\t\t\t" +
		...
		private void XMLFile(ListBox box)
		{
			...
			XmlAttribute idattr;
			// create main node
			mainNode = doc.GetElementsByTagName("lyra2")[0];

			IEnumerator en = box.Items.GetEnumerator();
			en.Reset();
			while (en.MoveNext())
			{
				Song song = (Song) en.Current;
				...
				{
					Translation curtrans = (Translation) transen.Value;
		...
		// back
		private string getBack(ListBox box, Song song)
		{
		...
		private bool SaveHTML(ListBox box)
		{
			try
			{
				...
				links.Write("<a href=\"front.html\" target=\"content\"><img src=\"logo.jpg\" border=\"0\" alt=\"lyra2 HTML Liedtexte\"/></a>" +
					"<br />\n\t<br />\n\t" + "<h1>Inhaltsverzeichnis</h1>\n\n\t");
				links.Write("<span class=\"smallgr\"><b>" + this.idtext + "</b></span><br />\n\t");
				links.Write("<span class=\"smallgr\">generated by lyra2 " + Util.GetDate() + "</span><br />\n\t<br />\n\t<br />\n\t");
				links.Write("<table>\n\t\t");

				// Create Inhalt
				StreamWriter front = File.CreateText(this.textBox3.Text + "\\front.html");
				front.Write(this.HTMLHEADER + this.CLOSEHEADER);
				front.Write("<body>\n\n\t<h2>lyra2 Liedtexte (HTML)</h2>\n\t<br />\n\t<br />\n\t");
				string fronttxt = "HTML Dateien, generiert von lyra2 " + Util.VER + Util.HTMLNL;
				fronttxt += "Datum: " + Util.GetDate() + Util.HTMLNL + Util.HTMLNL + Util.HTMLNL;
				fronttxt += "Generiert aus:" + Util.HTMLNL + "<b>" + this.idtext + "</b>" + Util.HTMLNL + Util.HTMLNL + "Beinhaltet ";
				switch (this.box.Items.Count)
				{
					...
					case 1:
						...
						break;
					default:
						fronttxt += "<b>" + this.box.Items.Count.ToString() + "</b> Liedtexte.";
						break;
				}
				...
				while (en.MoveNext())
				{
					...
					if (song != null)
					{
						...
						songhtml.Write("<body>\n\n\t" + Util.HTMLNL + Util.HTMLNL + Util.HTMLNL + this.createSongTable(song));
						songhtml.Write("</body>\n</html>");
						songhtml.Close();
						links.Write("<tr><td class=\"out\" onMouseOver=\"this.className='over'\" " +
							"onMouseOut=\"this.className='out'\" onClick=\"parent.frames['content'].location.href='" + song.ID +
							".html'\">");
		...
		private string XSL_2 =
			"\n    </span><br />\n<br />\n" +
				...
				"    <xsl:for-each select=\"/lyra2/Song\">\n" +
