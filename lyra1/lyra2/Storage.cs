...
namespace lyra2
{
	...
	public class Storage : IStorage
	{
		...
		// get Song by ID
		public Song getSongById(object id)
		{
		...
		public bool Export(string url)
		{
			try
			{
				System.IO.File.Copy(Util.BASEURL + "\\" + Util.URL, url, true);
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ExportPPC(string url)
		{
			try
			{
				...
				sw.AutoFlush = true;
				sw.WriteLine("[ lyra2 for Pocket PC ]");
				sw.WriteLine();
				...
				MessageBox.Show(this.owner, text, "lyra2", MessageBoxButtons.OK, MessageBoxIcon.Information);
				return true;
			}
			catch (Exception)
			{
