...
namespace lyra2
{
	...
	public class PhysicalXML : IPhysicalStorage
	{
			...
			try
			{
				...
				for (int i = 0; i < nodes.Count; i++)
				{
					...
					catch (ArgumentException)
					{
						...
						DialogResult dr = MessageBox.Show(msg, "lyra2 import",
		...
		private void GetTranslations(Song song, string translations, bool imp)
			// throws XmlException, NotValidException
		{
			if (translations == "")
				return;
			string[] trs = translations.Split(',');
			string title, text, id, newid;
			int lang;
			bool uf;
			XmlNode curt;
			for (int i = 0; i < trs.Length; i++)
			{
				if ((curt = doc.GetElementById(trs[i])) != null)
				{
					...
					lang = Util.getLanguageInt(curt.Attributes["lang"].Value);
					id = curt.Attributes["id"].Value;
					uf = curt.Attributes["unform"].Value.Equals("yes");
					...
					newid = PhysicalXML.HighestTrID;
					song.AddTranslation(new Translation(title, text, lang, uf, "t" + Util.toFour(highestTrID), imp || (newid != id)));
				}
				else
				{
			...
			try
			{
				...
				while (en.MoveNext())
				{
					...
					if (curSong.ToUpdate)
					{
						...
						// translations
						transList = curSong.UpdateTranslations(this);
			...
			// throws NotValidException, XmlException
		{
			...
			while (en.MoveNext())
			{
				...
				if (curTrans.ToUpdate)
				{
					if ((transNode = doc.GetElementById(curTrans.ID)) != null)
					{
						transChildren = transNode.ChildNodes;
						if ((transChildren[0].Name == "Title") &&
							(transChildren[1].Name == "Text"))
						{
	...
	public class NotValidException : System.Exception
	{
